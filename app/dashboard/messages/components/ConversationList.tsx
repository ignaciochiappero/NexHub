...
export function ConversationList({ conversations, selectedId, onSelect, onDelete, onNewChat }: ConversationListProps) {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-3 px-10">
      <AnimatePresence mode="wait">
        {conversations.map((conversation) => (
          <motion.div
            key={conversation.id}
            variants={itemVariants}
            className={`w-full px-4 py-3 rounded-xl flex items-center space-x-4 
          ...
          >
            <button onClick={() => onSelect(conversation.id)} className="flex-grow flex items-center">
              <div
              ...
              <div className="flex-1 text-left overflow-hidden ml-4">
                <p
                  className={`font-blender-medium truncate transition-colors duration-300
                   ${selectedId === conversation.id ? "text-pink-300" : "text-gray-200 group-hover:text-pink-300"}`}
                >
