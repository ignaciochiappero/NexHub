...
import HamburgerIcon from '../navbar/hambIcon/HamburguerIcon';
...
const RadialNavbar = () => {
  ...
  // Función simplificada para determinar la posición del tooltip
  const getTooltipPosition = (angle: number) => {
    // Solo el ícono a 0 grados (el más bajo) tendrá el tooltip hacia abajo
    if (angle === 0) {
      return 'top-full mt-2 left-1/2 -translate-x-1/2 -translate-y-3 group-hover:translate-y-0';
    }
    // El resto tendrá el tooltip hacia la izquierda
        ...
        menuItems.map(({ id, icon: Icon, angle, label, site }, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Tooltip adaptativo */}
              <div className={`absolute whitespace-nowrap px-2 py-1 text-base
